...
/* Keep the original classifier coverage without pulling in SDL private sources. */
...
static int
guess_device_class(const unsigned long bitmask_ev[NBITS(EV_MAX)],
                   ...
                   const unsigned long bitmask_rel[NBITS(REL_MAX)])
{
    ...
    if (test_bit(EV_ABS, bitmask_ev) &&
        ...
        if (test_bit(BTN_STYLUS, bitmask_key) || test_bit(BTN_TOOL_PEN, bitmask_key)) {
            ...
            devclass |= SDL_UDEV_DEVICE_TOUCHPAD;
...
run_test(void)
{
    ...
    for (i = 0; i < SDL_arraysize(guess_tests); i++) {
        ...
        actual = guess_device_class(caps.ev, caps.abs, caps.keys, caps.rel);
