...
static SDL_bool createNewSwapchainAndSwapchainSpecificStuff(void)
{
    ...
    createFences();
    vulkanContext->swapchainRebuildCount += 1;
    return SDL_TRUE;
}
...
static void initVulkan(void)
{
    ...
    if (SDL_Vulkan_LoadLibrary(NULL) < 0) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "SDL_Vulkan_LoadLibrary(): %s\n", SDL_GetError());
        quit(2);
    }
...
int main(int argc, char **argv)
{
    ...
    int initial_w;
    int initial_h;
    Uint32 initial_swapchain_rebuilds;
    SDL_bool requested_first_resize = SDL_FALSE;
    SDL_bool requested_second_resize = SDL_FALSE;
    SDL_bool saw_resize_rebuild = SDL_FALSE;
    ...
    while (!done && frames < TEST_VULKAN_MAX_FRAMES) {
        ...
        if (!requested_first_resize && frames == TEST_VULKAN_FIRST_RESIZE_FRAME) {
            SDL_SetWindowSize(state->windows[0],
                              initial_w + TEST_VULKAN_RESIZE_DELTA_W,
                              initial_h + TEST_VULKAN_RESIZE_DELTA_H);
            requested_first_resize = SDL_TRUE;
        }

        if (!requested_second_resize && frames == TEST_VULKAN_SECOND_RESIZE_FRAME) {
            SDL_SetWindowSize(state->windows[0], initial_w, initial_h);
            requested_second_resize = SDL_TRUE;
        }
    ...
    if (vulkanStalled) {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Automated resize did not rebuild the Vulkan swapchain\n");
        rc = 2;
    }
